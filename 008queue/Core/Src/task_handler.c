...
void menu_task(void *param)
{
    ...
    while(1)
    {
        ...
        if(cmd->len == 1)
        {
            ...
            switch(option)
            {
                ...
                case 2:
                    break;
                default:
                    xQueueSend(q_print, &msg_inv, portMAX_DELAY); //portMAX_DELAY infinite block
                    continue;
            }
        }
        else
        ...
        //wait to run again when some other task notify
...
void led_task(void *param)
{
	...
	while(1){
		...
		/*TODO: Print LED menu */
        xQueueSend(q_print, &msg_led, portMAX_DELAY); //portMAX_DELAY infinite block
...
void rtc_task(void *param)
{
	...
	while(1)
    {
        ...
        xQueueSend(q_print, &msg_rtc1, portMAX_DELAY); //portMAX_DELAY infinite block
        show_time_date();
        xQueueSend(q_print, &msg_rtc2, portMAX_DELAY); //portMAX_DELAY infinite block
...
int extract_command(command_t *cmd)
{
    uint8_t item;
    BaseType_t status;
    status = uxQueueMessagesWaiting(q_data); /*return valid numer of messages in the queue*/
